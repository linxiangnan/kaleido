 ...
 * @func
 **/
@Component(value = Constant.COMPONENT_RPC_DISPATCHER_NAME)
public class RpcDispatcher {
    ...
    @Autowired
    private InvokeUtils invokeUtils;

    private static Map<String, Map<String, Object>> methodMap = new HashMap();

    public Message callRpc(int requestId, String className, String methodName, Map body) throws Exception {
        ...
        if (methodMap.containsKey(className) && methodMap.get(className).containsKey(methodName)) {
            ...
            Object bean;
            try {
                bean = AppContext.getBean(invokeUtils.getGrpcBeanName(className));
            } catch (Exception e) {
